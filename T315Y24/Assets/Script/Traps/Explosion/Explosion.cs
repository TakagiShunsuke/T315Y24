...
public class Explosion : MonoBehaviour
{
    ...
    [Header("ステータス")]
    [SerializeField,Tooltip("爆発の見た目を小さくしていく速度")] private double m_LowerSpeed = 0.1d;   //オブジェクトが下に消えていく速度
    public static int[] m_KillCount = new int[2];   //罠ごとに倒した数を保存する配列
    int m_Setnum;   //格納先識別番号
    ...
    */
    ...
    /*＞格納先識別番号セット関数
    引数１：int _nNum：格納先の番号
    ｘ
    戻値：なし
    ｘ
    概要：討伐数の保存の指定場所をセットする
    */
    public void SetBombType(int _nNum)
    {
        m_Setnum = _nNum;   //格納先をセット
    }
}
